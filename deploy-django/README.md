# Deploying a Django App on Flux Network with Deploy With Git

This is a minimal Django 4 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Django App
...
3. Detects `manage.py` and runs `python manage.py runserver 0.0.0.0:$PORT`

> **Note:** For production deployments, override the run command with `gunicorn hello_flux.wsgi:application --bind 0.0.0.0:$PORT` using the `RUN_COMMAND` environment variable.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-django&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-django/` if it's inside a monorepo), then click **Continue**
...
Set this to `8000` (the port Django's dev server listens on). Flux will route external traffic to this port.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | None (no build step for Django) |
| `RUN_COMMAND` | Custom startup command | `python manage.py runserver 0.0.0.0:$PORT` |
| `WEBHOOK_SECRET` | Enables instant CI/CD via GitHub Webhooks | — |

You can also set Django-specific variables:

| Variable | Description | Default |
|---|---|---|
| `SECRET_KEY` | Django secret key for cryptographic signing | A default is set for demo purposes |
