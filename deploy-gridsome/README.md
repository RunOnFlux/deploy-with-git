# Deploying a Gridsome Site on Flux Network with Deploy With Git

This is a minimal Gridsome 0.7 "Hello World" site ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Gridsome Site
...
Gridsome compiles your Vue components into a fully static bundle of HTML, CSS, and JS in the `dist/` folder. Flux serves this using its built-in static file server — no separate Node.js process is needed at runtime.

> **Note:** Gridsome uses webpack 4 which requires `NODE_OPTIONS=--openssl-legacy-provider` on Node.js 17+. This is already configured in the `build` script.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-gridsome&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-gridsome/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects your runtime from your repository. Only fill this in if you need a specific version. Select **Node.js** and enter a version (e.g., `18`, `20`, `22` — LTS versions recommended). This project includes a `.nvmrc` file pinning Node 18, so no manual selection is needed.
