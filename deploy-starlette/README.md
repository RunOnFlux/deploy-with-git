# Deploying a Starlette App on Flux Network with Deploy With Git

This is a minimal Starlette "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Starlette App
...
3. Detects `main.py` and runs `python main.py`

The app uses [Uvicorn](https://www.uvicorn.org/) as the ASGI server, started directly from `main.py`.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-starlette&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-starlette/` if it's inside a monorepo), then click **Continue**
...
Set this to `8000` (the port Uvicorn listens on). Flux will route external traffic to this port.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | None (no build step for Starlette) |
| `RUN_COMMAND` | Custom startup command | `python main.py` |
