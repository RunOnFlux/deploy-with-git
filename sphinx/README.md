...
4. Detects `app.py` as the entry point and runs it — `app.py` serves the `dist/` directory using Python's built-in HTTP server
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://orbit.app.runonflux.io/?repo=https://github.com/RunOnFlux/deploy-with-git-samples&branch=master&projectPath=sphinx&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `sphinx/` if it's inside a monorepo), then click **Continue**
...
This generates the `dist/` directory. At runtime, `app.py` serves it using Python's built-in HTTP server.
