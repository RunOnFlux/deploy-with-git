# Deploying a VitePress Site on Flux Network with Deploy With Git

This is a minimal VitePress "Hello World" site ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your VitePress Site
...
VitePress generates a fully static site in the `dist/` folder. Orbit detects the `dist/index.html` output and serves it using its built-in static file server — no `start` script or separate Node.js process is needed at runtime.
...
Add more `.md` files inside `docs/` to create additional pages.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-vitepress&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-vitepress/` if it's inside a monorepo), then click **Continue**
...
Orbit serves static sites on port `80` by default using its built-in static file server. No `start` script or port configuration is required.
...
> **Note:** VitePress requires Node.js 18 or later. Node 20 LTS is recommended.
