# Deploying a Solid.js App on Flux Network with Deploy With Git

This is a minimal Solid.js "Hello World" app (built with Vite) ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Solid.js App
...
Solid.js with Vite compiles to a fully static bundle of HTML, CSS, and JS in the `dist/` folder. The `start` script runs `vite preview` which serves the static output — Flux keeps this process running on the configured port.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://orbit.app.runonflux.io/?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-solidjs&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-solidjs/` if it's inside a monorepo), then click **Continue**
...
The `start` script defaults to port `3000` and reads the `PORT` environment variable automatically. Flux will pre-fill this.
...
> **Note:** Solid.js with Vite requires Node.js 18 or later. Node 20 LTS is recommended.
