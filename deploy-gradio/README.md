# Deploying a Gradio App on Flux Network with Deploy With Git

This is a minimal Gradio "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Gradio App
...
3. Detects `gradio_app.py` and runs `python gradio_app.py`
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-gradio&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-gradio/` if it's inside a monorepo), then click **Continue**
...
Set this to `7860` (Gradio's default port). Flux will route external traffic to this port.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | None (no build step for Gradio) |
| `RUN_COMMAND` | Custom startup command | `python gradio_app.py` |
...
Open [http://localhost:7860](http://localhost:7860) to see the app.
