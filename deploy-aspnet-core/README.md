# Deploying an ASP.NET Core App on Flux Network with Deploy With Git

This is a minimal ASP.NET Core "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your ASP.NET Core App
...
5. Runs the app with `dotnet deploy-aspnet-core.dll`
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-aspnet-core&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-aspnet-core/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the .NET version from `TargetFramework` in `deploy-aspnet-core.csproj`. Only fill this in if you need a specific version. This project targets `net8.0`, so no manual selection is needed.
