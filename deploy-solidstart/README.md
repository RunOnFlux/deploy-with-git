# Deploying a SolidStart App on Flux Network with Deploy With Git

This is a minimal SolidStart 1 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your SolidStart App
...
SolidStart uses [Vinxi](https://vinxi.vercel.app/) to build a server bundle to `.output/server/index.mjs` and client assets to `.output/public/`. The `start` script runs `node .output/server/index.mjs` which spins up a Node.js HTTP server — Flux detects this as `serve type: node` and keeps the process running.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-solidstart&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `deploy-solidstart/` if it's inside a monorepo), then click **Continue**
...
SolidStart's node server defaults to port `3000` and reads the `PORT` environment variable automatically. Flux will pre-fill this.
...
> **Note:** SolidStart requires Node.js 20 or later.
