...
This is a minimal Next.js "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://orbit.app.runonflux.io/) feature.
...
## How Flux Detects Your Next.js App
...
The `output: "standalone"` option in `next.config.ts` is recommended — it produces a self-contained build that starts faster and uses less memory on Flux nodes.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://orbit.app.runonflux.io/?repo=https://github.com/RunOnFlux/deploy-with-git-samples&branch=master&projectPath=nextjs&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** to the folder (e.g., `nextjs/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the port from your framework. For Next.js it will show **Auto-detected from Next.js** and pre-fill `3000`. Only change this if your app listens on a different port.

The `start` script in `package.json` uses the `PORT` environment variable so Flux can pass in the correct port at runtime:
