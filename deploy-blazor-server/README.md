# Deploying a Blazor Server App on Flux Network with Deploy With Git

This is a minimal Blazor Server "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Blazor Server App

Flux identifies .NET projects by looking for `*.csproj` files. It then:

1. Reads the .NET version from `global.json`, `TargetFramework` in the `.csproj`, or the `DOTNET_VERSION` environment variable (defaults to .NET 8.0 LTS)
2. Installs the .NET SDK
3. Restores dependencies via `dotnet restore`
4. Publishes with `dotnet publish -c Release -o ./publish`
5. Runs the app with `dotnet deploy-blazor-server.dll`
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-blazor-server&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-blazor-server/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the .NET version from `TargetFramework` in `deploy-blazor-server.csproj`. Only fill this in if you need a specific version. This project targets `net8.0`, so no manual selection is needed.
