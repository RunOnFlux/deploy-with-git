# Deploying a TanStack Start App on Flux Network with Deploy With Git

This is a minimal TanStack Start 1 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your TanStack Start App
...
TanStack Start uses [Vite](https://vite.dev/) to compile your app into a server bundle at `dist/server/server.js` and client assets at `dist/client/`. A `post-build.sh` hook generates a lightweight Node.js server wrapper at `dist/server-node.mjs` using [srvx](https://srvx.unjs.io/). The `start` script runs this wrapper — Flux detects this as `serve type: node` and keeps the process running.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-tanstack-start&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `deploy-tanstack-start/` if it's inside a monorepo), then click **Continue**
...
The node server defaults to port `3000` and reads the `PORT` environment variable automatically. Flux will pre-fill this.
...
> **Note:** TanStack Start requires Node.js 20 or later.
...
This project already uses a `post-build.sh` hook to generate the Node.js server wrapper. You can extend it with your own logic:
