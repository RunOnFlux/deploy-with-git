# Deploying a LoopBack 4 App on Flux Network with Deploy With Git

This is a minimal LoopBack 4 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your LoopBack App
...
LoopBack 4 compiles TypeScript to JavaScript using `lb-tsc`. The output is configured to go to `dist/server/` (via `tsconfig.json`), which Flux recognises as a Node.js server entry point. Flux then runs it as a persistent process.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-loopback&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-loopback/` if it's inside a monorepo), then click **Continue**
...
Set this to `3000` (the port LoopBack listens on). Flux will route external traffic to this port.
