# Deploying a Nuxt App on Flux Network with Deploy With Git

This is a minimal Nuxt 3 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Nuxt App
...
Nuxt's build outputs a self-contained Nitro server to `.output/server/index.mjs`. The `start` script points directly to this file so Flux recognises it as a dynamic Node.js server:
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-nuxt&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `deploy-nuxt/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the port from your framework. For Nuxt it will pre-fill `3000`. Only change this if your app listens on a different port.

Nuxt's Nitro server reads the `PORT` environment variable automatically at runtime, so no extra configuration is needed on your end.
...
| `NUXT_PUBLIC_*` | Public runtime config variables | — |
