# Deploying a Koa App on Flux Network with Deploy With Git

This is a minimal Koa "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
Koa starts a long-running HTTP server process. Flux detects this as `serve type: node` and keeps the process alive, restarting it automatically if it crashes.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-koa&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-koa/` if it's inside a monorepo), then click **Continue**
...
Koa reads the `PORT` environment variable and defaults to `3000`. Flux will pre-fill this.
...
> **Note:** Koa requires Node.js 12 or later and makes full use of async/await. Node 20 LTS is recommended.
