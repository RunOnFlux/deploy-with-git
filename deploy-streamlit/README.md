# Deploying a Streamlit App on Flux Network with Deploy With Git

This is a minimal Streamlit "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Streamlit App
...
3. Detects `streamlit_app.py` and runs `streamlit run streamlit_app.py --server.port=$PORT --server.address=0.0.0.0`
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-streamlit&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-streamlit/` if it's inside a monorepo), then click **Continue**
...
Set this to `8501` (Streamlit's default port). Flux will route external traffic to this port.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | None (no build step for Streamlit) |
| `RUN_COMMAND` | Custom startup command | `streamlit run streamlit_app.py --server.port=$PORT --server.address=0.0.0.0` |
...
Open [http://localhost:8501](http://localhost:8501) to see the app.
