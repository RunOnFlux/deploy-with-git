...
5. Runs the app with `dotnet blazor-server.dll`
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://orbit.app.runonflux.io/?repo=https://github.com/RunOnFlux/deploy-with-git-samples&branch=master&projectPath=blazor-server&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `blazor-server/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the .NET version from `TargetFramework` in `blazor-server.csproj`. Only fill this in if you need a specific version. This project targets `net8.0`, so no manual selection is needed.
