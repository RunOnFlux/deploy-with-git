# Deploying a Qwik City App on Flux Network with Deploy With Git

This is a minimal Qwik City 1 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Qwik City App
...
The build runs two Vite passes: a client build that outputs assets to `dist/`, and a server build via the node-server adaptor that outputs a self-contained SSR bundle to `dist/server/`. The `start` script runs `node dist/server/entry.node-server.js` — Flux detects this as `serve type: node` and keeps the process running.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://orbit.app.runonflux.io/?repo=https://github.com/RunOnFlux/deploy-with-git-samples&branch=master&projectPath=qwikcity&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `qwikcity/` if it's inside a monorepo), then click **Continue**
