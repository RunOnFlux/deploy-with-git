# Deploying a Micronaut App on Flux Network with Deploy With Git

This is a minimal Micronaut "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Micronaut App

Flux identifies Java projects by looking for `pom.xml`. It then:

1. Reads the Java version from `.java-version` or the `JAVA_VERSION` environment variable (defaults to Java 21 LTS)
2. Downloads Eclipse Temurin JDK and installs Maven 3.9.6
3. Compiles with `mvn clean package -DskipTests`
4. Locates the fat JAR in `target/`
5. Runs it with `java -jar app.jar`

The server port is configured via the `PORT` environment variable, which Flux sets automatically at runtime.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-micronaut&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-micronaut/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the Java version from `.java-version` in your repository. Only fill this in if you need a specific version. This project includes a `.java-version` file pinning Java 21, so no manual selection is needed.
...
| `JAVA_VERSION` | Java version to use | `21` (from `.java-version`) |
| `JAVA_OPTS` | Custom JVM options | auto-configured |
| `BUILD_COMMAND` | Custom build command | `mvn clean package -DskipTests` |
...
- [Deploying Java Apps on Flux](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/guides/deploying-java)
