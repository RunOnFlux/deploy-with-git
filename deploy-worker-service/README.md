# Deploying a .NET Worker Service on Flux Network with Deploy With Git

This is a minimal .NET Worker Service ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.

It demonstrates a background worker (`IHostedService`) running alongside a minimal HTTP endpoint — the standard pattern for cloud-deployed worker services that need liveness/readiness probes.
...
## How Flux Detects Your Worker Service
...
4. Publishes with `dotnet publish -c Release -o ./publish`
5. Runs the app with `dotnet deploy-worker-service.dll`

The server binds to `0.0.0.0:$PORT` automatically via the `ASPNETCORE_URLS` environment variable set by Flux.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-worker-service&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-worker-service/` if it's inside a monorepo), then click **Continue**
...
Flux auto-detects the .NET version from `TargetFramework` in `deploy-worker-service.csproj`. Only fill this in if you need a specific version. This project targets `net8.0`, so no manual selection is needed.
...
Open [http://localhost:8080](http://localhost:8080) to see the app. Check your terminal for the worker log output.
