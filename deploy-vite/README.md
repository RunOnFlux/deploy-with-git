# Deploying a Vite App on Flux Network with Deploy With Git

This is a minimal Vite "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Vite App
...
Vite's build output is a static bundle of HTML, CSS, and JS written to `dist/`. The `start` script uses `vite preview` to serve that bundle in production:

```json
"start": "vite preview --port ${PORT:-4173} --host"
```

The `--host` flag is required so the server binds to `0.0.0.0` and is reachable on the network — not just `localhost`.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-vite&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `deploy-vite/` if it's inside a monorepo), then click **Continue**
...
Vite's preview server defaults to port `4173`. Flux will pre-fill this automatically. Only change it if you configure a different port in `vite.config.js`.

The `start` script reads the `PORT` environment variable so Flux can pass in the correct port at runtime:
