# Deploying an Actix-web App on Flux Network with Deploy With Git

This is a minimal Actix-web "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Actix-web App

Flux identifies Rust projects by looking for `Cargo.toml`. It then:

1. Reads the Rust version from `rust-toolchain.toml` or the `RUST_VERSION` environment variable (defaults to `stable`)
2. Compiles the app with `cargo build --release`
3. Runs the compiled binary from `target/release/`
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-actix-web/` if it's inside a monorepo), then click **Continue**
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-actix-web&plan=free)
...
Flux reads the Rust version from `rust-toolchain.toml`. Only fill this in if you need to override it. This project pins `stable`, so no manual selection is needed.
...
| `RUST_VERSION` | Rust toolchain version | `stable` (from `rust-toolchain.toml`) |
| `BUILD_COMMAND` | Custom build command | `cargo build --release` |
...
- [Deploying Rust Apps on Flux](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/guides/deploying-rust)
