# Deploying a Falcon App on Flux Network with Deploy With Git

This is a minimal Falcon 3 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Falcon App
...
The app uses Python's built-in `wsgiref` server to serve requests. For production deployments, override the run command with gunicorn using the `RUN_COMMAND` environment variable.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-falcon&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-falcon/` if it's inside a monorepo), then click **Continue**
...
Set this to `8000` (the port the app listens on). Flux will route external traffic to this port.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | None (no build step for Falcon) |
