# Deploying a Hapi App on Flux Network with Deploy With Git

This is a minimal Hapi "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Hapi App
...
Hapi starts a long-running HTTP server process. Flux detects this as `serve type: node` and keeps the process alive, restarting it automatically if it crashes.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-hapi&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-hapi/` if it's inside a monorepo), then click **Continue**
...
Hapi reads the `PORT` environment variable and defaults to `3000`. The server is bound to `0.0.0.0` so it is reachable inside the container. Flux will pre-fill the port automatically.
...
> **Note:** `@hapi/hapi` v21+ requires Node.js 20 or later.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | Auto-detected from `package.json` scripts |
