# Deploying an Express App on Flux Network with Deploy With Git

This is a minimal Express "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Express App
...
3. Runs `npm install` → `npm run start`

Express has no build step — Flux installs dependencies and starts the server directly. It detects Express as a web framework and classifies it as `serve type: node`.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-express&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `deploy-express/` if it's inside a monorepo), then click **Continue**
...
The server defaults to port `3000` and reads the `PORT` environment variable automatically:

```js
const port = process.env.PORT || 3000;
```

Flux will pre-fill `3000`. Only change this if your app listens on a different port.
...
|---|---|---|
| `BUILD_COMMAND` | Custom build command | None (Express has no build step) |
| `RUN_COMMAND` | Custom startup command | Auto-detected from `package.json` `start` script |
...
Every push to your branch will now trigger an automatic redeploy (~2 minutes). Flux automatically rolls back to the previous version if the new deployment fails.
