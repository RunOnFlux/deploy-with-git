# Deploying a Remix App on Flux Network with Deploy With Git

This is a minimal Remix 2 "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Remix App
...
Remix builds a server bundle to `build/server/index.js` and client assets to `build/client/`. The `start` script runs `remix-serve` which spins up a Node.js HTTP server — Flux detects this as `serve type: node` and keeps the process running.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-remix&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path**  (e.g., `deploy-remix/` if it's inside a monorepo), then click **Continue**
...
`remix-serve` defaults to port `3000` and reads the `PORT` environment variable automatically. Flux will pre-fill this.
