# Deploying a Rails App on Flux Network with Deploy With Git

This is a minimal Rails "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Rails App

Flux identifies Ruby projects by looking for a `Gemfile`. It then:

1. Reads the Ruby version from `.ruby-version` or the `RUBY_VERSION` environment variable (defaults to 3.2.0)
2. Installs Ruby via rbenv
3. Installs dependencies via `bundle install`
4. Detects `config.ru` and runs the app with `bundle exec puma -p $PORT`
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-rails&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-rails/` if it's inside a monorepo), then click **Continue**
...
Flux defaults to Ruby 3.2.0. Pin a specific version by adding a `.ruby-version` file to your repository or setting the `RUBY_VERSION` environment variable.
...
| `RUBY_VERSION` | Ruby version to use | `3.2.0` |
...
- [Deploying Ruby Apps on Flux](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/guides/deploying-ruby)
