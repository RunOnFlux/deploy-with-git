# Deploying a Lit App on Flux Network with Deploy With Git

This is a minimal Lit "Hello World" app (built with Vite) ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
Lit with Vite compiles to a fully static bundle of HTML, CSS, and JS in the `dist/` folder. Orbit detects the `dist/index.html` output and serves it using its built-in static file server — no `start` script or separate Node.js process is needed at runtime.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-lit&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-lit/` if it's inside a monorepo), then click **Continue**
