# Deploying a Restify App on Flux Network with Deploy With Git

This is a minimal Restify "Hello World" app ready to be deployed on [Flux Network](https://runonflux.com) using the [Deploy With Git](https://docs.runonflux.com/fluxcloud/register-new-app/deploy-with-git/) feature.
...
## How Flux Detects Your Restify App
...
Restify starts a long-running HTTP server process. Flux detects this as `serve type: node` and keeps the process alive, restarting it automatically if it crashes.
...
[<img width="140" height="28" alt="deploy3" src="https://github.com/user-attachments/assets/782abbc4-106a-4221-b87c-2879d5cc3400" />](https://cloud.runonflux.io/apps/register/orbit?repo=https://github.com/RunOnFlux/deploy-with-git&branch=master&projectPath=deploy-restify&plan=free)
...
4. Paste your **Repository URL**, select your **branch** and the **path** (e.g., `deploy-restify/` if it's inside a monorepo), then click **Continue**
...
Restify reads the `PORT` environment variable and defaults to `3000`. Flux will pre-fill this.
